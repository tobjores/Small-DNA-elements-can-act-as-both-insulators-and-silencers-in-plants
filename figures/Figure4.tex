\begin{tikzpicture}
	...
	% names
	\node[anchor = south, node font = \figlarge\bfseries, shift = {(.5\fourcolumnwidth, .25\columnsep)}] at (constructs iDE |- lwith.north) {with 35S};
	\node[anchor = north, node font = \figlarge\bfseries, shift = {(.5\fourcolumnwidth, -.25\columnsep)}] at (constructs iDE |- lwithout.south) {without 35S};
	...
	\begin{pggfplot}[%
		...
		xlabel = {\textbf{without \usename{35S}:} \enrichment},
		ylabel = {\textbf{with \usename{35S}:} \enrichment},
		title is csname,
		title color from name,
	]{iDE_cor_35S}
